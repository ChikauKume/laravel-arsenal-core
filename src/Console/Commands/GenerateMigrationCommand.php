...
class GenerateMigrationCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        if (!empty($existingTables)) {
            $this->warn("The following tables already exist:");
            foreach ($existingTables as $table) {
                $this->line(" - {$table}");
            }
            
            if ($this->option('force') || $this->confirm('Do you want to drop and recreate these tables?')) {
     ...
     * @return void
     */
    protected function showPreview(array $tables): void
    {
        $this->info("The following tables will be generated:");
        ...
        foreach ($tables as $table) {
            ...
            foreach ($table['columns'] as $column) {
                ...
                $modifiers = [];
                if ($column['primary']) $modifiers[] = 'PRIMARY';
                if (!$column['nullable']) $modifiers[] = 'NOT NULL';
    ...
    /**
     * Extract size from column definition
     *
     * @param string $sizeStr
     * @return string|null
     */
    protected function extractSize(string $sizeStr): ?string
    {
        if (preg_match('/\((\d+)\)/', $sizeStr, $matches)) {
            return $matches[1];
        }
        return null;
    }
